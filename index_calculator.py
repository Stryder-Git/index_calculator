...
class IndexCalculator:
    ...
    * how about continuous?
    ...
    @contextmanager
    def _temp(self, frequency, frm, to):
        """frm and to can never be None, but frequency can"""
        _sched = self.schedule.real.ge(frm.normalize()) & self.schedule.end.lt(to.normalize() + self._day)
        if frequency is None:
            self._sched = self.schedule[_sched]
            yield;
            return

        original_freq = self._freq
        original_start = self.schedule.start.copy()
    ...
    def _times(self):
        ...
        ts = np.ceil((self._sched.end - self._sched.start) / self.frequency)
        ts = self._sched.index.repeat(ts)
        ts = self._sched.reindex(ts).reset_index(drop=True)

        ts["target"] = (ts.start.groupby(ts.real).cumcount() + 1) * self.frequency + ts.start
        if self._adj:
            return self._handle_start_end(ts).sort_values().reset_index(drop=True)
        else:
            ts.target.name = None
            return ts.target.reset_index(drop=True)
    ...
    def times(self, frequency=None, frm=None, to=None, tz=None):
        ...
        if tz is None:
            if frm is None:
                ...
                try:
                    frm = frm.tz_localize(self.SCHEDTZ)
                except TypeError:
                    pass
            ...
            if to is None:
                ...
                to = pd.Timestamp(to)
                try:
                    to = to.tz_localize(self.SCHEDTZ)
                except TypeError:
                    pass

            with self._temp(frequency, frm, to):
                return self._times()
            ...
            if frm is None:
                ...
                frm = pd.Timestamp(frm)
                try:
                    frm = frm.tz_localize(tz).tz_convert(self.SCHEDTZ)
                except TypeError:
                    frm = frm.tz_convert(self.SCHEDTZ)
            ...
            if to is None:
                ...
                to = pd.Timestamp(to)
                try:
                    to = to.tz_localize(tz).tz_convert(self.SCHEDTZ)
                except TypeError:
                    to = to.tz_convert(self.SCHEDTZ)

            with self._temp(frequency, frm, to):
                return self._times().dt.tz_convert(tz)

    def timex(self, frequency=None, frm=None, to=None, tz=SCHEDTZ):
    ...
    def _resample(self, firstrows, agg_map, label):
        ...
        frequency = self.frequency  # to avoid going through @property every time

        def __new(df):
            return df.resample(frequency, origin=origins.at[df.index[0]], label=label
                               ).agg(agg_map)
    ...
    def _convert(self, data, agg_map, closed):
        ...
        if self.align:
            ...
            data[self._srt] = np.arange(data.shape[0])
            ...
            new = new.set_index(pd.DatetimeIndex(self._times()), drop=True
    ...
    def convert(self, data, freq=None, agg_map=None, closed="left", tz=None):
        ...
        with self._temp(freq, self.__frm, self.__to):
